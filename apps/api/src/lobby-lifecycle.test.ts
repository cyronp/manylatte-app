...
it('reports readable but unwritable storage as unavailable and recovers', async () => {
  ...
  try {
    ...
    await database.$executeRawUnsafe(
      "CREATE TRIGGER fail_probe BEFORE UPDATE ON _StorageProbe BEGIN SELECT RAISE(ABORT, 'write denied'); END",
    );
    await storage.check();
