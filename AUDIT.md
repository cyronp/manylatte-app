...
| F04           | Controlled node synchronization for keyboard movement/deletion and all final group positions                                                                                                                        | Two-client keyboard/deletion/reload and group-drag persistence tests                                   |
...
| F23           | Responsive 320px code input, reduced motion, removal of inert ScreenShare; documented connection-scoped identity and saved color preference. Creation remains in the canvas context menu without floating controls. | Browser layout/context-menu tests and documented identity contract                                     |
...
Final local validation: clean `npm ci` completed; the newly reported Hono and js-yaml advisories were then fixed with compatible lockfile updates. Forced full build and test runs passed: 107 unit/integration tests, three Chromium tests, and actual Prisma migration/backup recovery smoke checks. Lint, formatting and the dependency audit passed with zero reported vulnerabilities.
